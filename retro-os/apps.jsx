...
const PERSONA = {
  ...
  blurb:
    "I build fast, dependable software — full-stack web apps, developer tooling, and the systems that hold them together. Ten years shipping production code and sweating the details users never notice but always feel.",
  location: "Oslo, NO",
...
const PROJECTS = [
  {
    ...
    icon: "📼",
    tag: "Lead",
    summary: "A 200-component, fully-tokenized system powering 9 products. Cut design-to-ship time 40%.",
    stack: ["React", "TypeScript", "Tokens Studio", "Storybook"],
    color: "#b5327e",
    detail:
      "Owned the architecture: primitives vs. patterns split, a three-tier token pipeline (global → alias → component), and a contribution model that let 30 engineers ship components safely. Shipped dark mode across the suite in one release.",
  },
  {
    id: "p2",
    name: "Phosphor",
    ...
    icon: "🖥️",
    tag: "Solo",
    summary: "A terminal-first prototyping playground. 12k weekly users, 4.9★.",
    stack: ["Rust", "WebGL", "WASM"],
    color: "#2d6a8e",
    detail:
      "Real-time canvas with a scripting layer; renders 60fps on a decade-old laptop. Built the WASM bridge and the plugin API used by 40+ community extensions.",
  },
  {
    id: "p3",
    name: "Relay",
    ...
    icon: "📡",
    tag: "Tech lead",
    summary: "An event-streaming gateway handling 80k msg/sec at four nines of uptime.",
    stack: ["Go", "Kafka", "gRPC", "k8s"],
    color: "#1f8a6e",
    detail:
      "Designed the partitioning and back-pressure model, cut p99 latency from 240ms to 38ms, and wrote the replay tooling the on-call team actually likes using.",
  },
  {
    id: "p4",
    name: "Ledger95",
    ...
    icon: "💾",
    tag: "Solo",
    summary: "A retro budgeting app that looks like a 90s desktop (yes, like this one).",
    stack: ["Svelte", "SQLite", "Tauri"],
    color: "#7a3a9e",
    detail:
      "Local-first finance app. The skeuomorphic ledger book and the chunky calculator are the whole pitch — and people love them.",
  },
];
...
function AboutSection() {
  return (
    ...
    >
      ...
      <div>
        <h2 className="chrome" style={{ margin: "0 0 2px", fontSize: 24 }}>
          {PERSONA.name}
        </h2>
        <div className="mono" style={{ fontSize: 16, color: "var(--accent-2)", marginBottom: 10 }}>
          {PERSONA.role}
        </div>
        ...
        <GroupBox key={g.group} label={g.group}>
          <div style={{ display: "grid", gap: 9 }}>
            {g.items.map(([name, val]) => (
              <div
                key={name}
                style={{ display: "grid", gridTemplateColumns: "130px 1fr 42px", gap: 10, alignItems: "center" }}
              >
                ...
                <span className="mono" style={{ fontSize: 14, textAlign: "right", color: "var(--text-dim)" }}>
                  {val}%
                </span>
              </div>
            ))}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      >
        <div
          ...
          style={{
            ...
            backgroundImage: "repeating-linear-gradient(45deg, rgba(255,255,255,0.12) 0 1px, transparent 1px 10px)",
        ...
        <div style={{ position: "absolute", left: 12, bottom: 10, display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div className="chrome" style={{ fontSize: 22, color: "#fff", textShadow: "1px 1px 0 rgba(0,0,0,0.4)" }}>
              {p.name}
            </div>
            <div className="mono" style={{ fontSize: 14, color: "rgba(255,255,255,0.85)" }}>
              {p.kind} · {p.year} · {p.tag}
            </div>
          </div>
        </div>
      ...
      <div style={{ padding: "var(--sp-4)", display: "grid", gap: 12 }}>
        ...
        <p style={{ margin: 0, fontSize: 14, lineHeight: 1.55, color: "var(--text-dim)", textWrap: "pretty" }}>
          {p.detail}
        </p>
...
function FileExplorer({ onLaunch, initialPath = "C:\\" }) {
  ...
  const iconFor = (e) =>
    e.type === "folder" ? "📁" : { pdf: "📄", txt: "📃", exe: "⚙️", prj: "🗂️", dat: "📊", bmp: "🖼️" }[e.ext] || "📄";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      >
        ...
        <div
          className="bevel-in-thin mono"
          style={{ flex: 1, padding: "3px 8px", background: "var(--surface-3)", fontSize: 14 }}
        >
          {crumb.join("\\")}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "130px 1fr", flex: 1, minHeight: 0 }}>
        <div
          className="bevel-in retro-scroll"
          style={{ background: "var(--surface-3)", padding: 6, overflow: "auto", margin: 3, marginRight: 0 }}
        >
        ...
        <div
          ...
          style={{
            ...
            alignContent: "start",
            display: "grid",
            gridTemplateColumns: "repeat(auto-fill, 86px)",
            gap: 4,
          }}
          onClick={() => setSel(null)}
        >
            ...
            return (
              <button
                ...
                style={{
                  ...
                  gap: 3,
                  padding: "6px 2px",
                  background: isSel ? "var(--selection-bg)" : "transparent",
                  border: "1px dotted transparent",
                  borderColor: isSel ? "var(--bevel-light)" : "transparent",
                  cursor: "pointer",
                }}
              >
                ...
                <span
                  style={{
                    ...
                    color: isSel ? "var(--selection-text)" : "var(--text)",
                  }}
                >
                  {e.name}
                </span>
...
/* ============================================================
   ...
   ============================================================ */
const TERM_BANNER = ["RetroOS Terminal [Version 1.0.95]", "(c) " + PERSONA.name + ". Type 'help' for commands.", ""];
function TerminalPanel({ onLaunch, onTheme }) {
  ...
  const run = (raw) => {
    ...
    switch ((c || "").toLowerCase()) {
      ...
      case "help":
        ...
        break;
      case "whoami":
        print([`${PERSONA.name} — ${PERSONA.role}`, PERSONA.blurb, ""]);
        break;
      case "projects":
        print([
          ...PROJECTS.map((p) => `  ${p.icon}  ${p.name.padEnd(10)} ${p.kind} (${p.year})`),
          "",
          "→ run 'open <name>' for details",
          "",
        ]);
        break;
      case "open": {
        ...
        if (p) {
          onLaunch("project", p.id);
          print([`Opening ${p.name}…`, ""]);
        } else if (["about", "skills", "contact", "files", "readme"].includes(q)) {
          onLaunch(q);
          print([`Opening ${q}…`, ""]);
      ...
      case "contact":
        onLaunch("contact");
        print(["Launching contact terminal…", ""]);
        break;
      case "ls":
        print([FILES["C:\\"].map((e) => e.name).join("   "), ""]);
        break;
      case "cat":
        print([args[0] === "readme.txt" ? PERSONA.blurb : `cat: ${args[0] || ""}: try 'ls'`, ""]);
        break;
      case "theme": {
        const m = (args[0] || "").toLowerCase();
        if (m.startsWith("d")) {
          onTheme("dark");
          print(["→ dark CRT theme", ""]);
        } else if (m.startsWith("l")) {
          onTheme("light");
          print(["→ light system theme", ""]);
      ...
      case "neofetch":
        ...
        break;
      case "sudo":
        print([{ t: "Nice try. You already have root here. 😉", tone: "warn" }, ""]);
        break;
      case "clear":
        setLines([]);
        return;
      default:
        print([{ t: `command not found: ${c}. type 'help'.`, tone: "err" }, ""]);
    }
  };
  ...
  const toneColor = (t) =>
    t === "err" ? "var(--err)" : t === "warn" ? "var(--warn)" : t === "accent" ? "var(--accent-2)" : "inherit";
    ...
    >
      ...
      >
        {lines.map((l, i) => (
          <div
            key={i}
            style={{ color: l.prompt ? "#9fffce" : toneColor(l.tone), whiteSpace: "pre-wrap", wordBreak: "break-word" }}
          >
            {l.t}
          </div>
        ))}
...
function ContactTerminal() {
  ...
  if (sent)
    return (
      <div
        className="mono"
        style={{ padding: "var(--sp-5)", textAlign: "center", display: "grid", gap: 10, placeItems: "center" }}
      >
  ...
  return (
    <div style={{ padding: "var(--sp-4)", display: "grid", gap: 12 }}>
      <div className="mono" style={{ fontSize: 15, color: "var(--accent-2)" }}>
        {">"} establishing connection to {PERSONA.email}…
      </div>
      <Field label="Name" err={err.name}>
        <input value={f.name} onChange={set("name")} className="bevel-in-thin retro-input" style={inputStyle} />
      </Field>
      <Field label="Email" err={err.email}>
        <input
          value={f.email}
          onChange={set("email")}
          className="bevel-in-thin retro-input"
          style={inputStyle}
          placeholder="you@domain.com"
        />
      </Field>
      <Field label="Message" err={err.msg}>
        <textarea
          value={f.msg}
          onChange={set("msg")}
          rows={4}
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
        ...
        <RetroButton variant="primary" onClick={submit}>
          ▶ Transmit
        </RetroButton>
      </div>
    </div>
...
const inputStyle = {
  ...
  border: "none",
  outline: "none",
  boxShadow: "var(--field-glow)",
};
function Field({ label, err, children }) {
  return (
    <label style={{ display: "grid", gap: 4 }}>
      <span style={{ fontSize: 13, fontWeight: 600 }}>
        {label} {err && <span style={{ color: "var(--err)", fontWeight: 400 }}>— {err}</span>}
      </span>
      {children}
    </label>
  );
