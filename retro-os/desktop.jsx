...
function useWindowManager() {
    ...
    (app, payload) => {
      setWins((ws) => {
        ...
        const meta =
          app === "project"
            ? (() => {
                const p = PROJECTS.find((x) => x.id === payload);
                return { title: `${p.name} — ${p.kind}`, icon: p.icon, w: 460, h: 440 };
              })()
            : APP_REGISTRY[app];
        ...
        return [
          ...ws,
          {
            ...
            title: meta.title,
            icon: meta.icon,
            x: Math.max(8, cx0 + off - 60),
            y: Math.max(8, cy0 + off - 40),
            w: meta.w,
            h: meta.h,
            z: nz,
            minimized: false,
  ...
  const minimize = useCallback(
    (id) => setWins((ws) => ws.map((w) => (w.id === id ? { ...w, minimized: true } : w))),
    [],
  );
  const toggleMax = useCallback(
    (id) => setWins((ws) => ws.map((w) => (w.id === id ? { ...w, maximized: !w.maximized } : w))),
    [],
  );
...
function WindowContent({ win, wm, onTheme }) {
  switch (win.app) {
    case "hero":
      return <WelcomeWindow onLaunch={wm.open} />;
    case "about":
      return <AboutSection />;
    case "skills":
      return <SkillsMatrix />;
    ...
    case "terminal":
      return <TerminalPanel onLaunch={wm.open} onTheme={onTheme} />;
    case "files":
      return <FileExplorer onLaunch={wm.open} />;
    case "projects":
      return <FileExplorer onLaunch={wm.open} initialPath="Projects" />;
    case "readme":
      return <ReadmeDoc />;
    case "resume":
      return <ResumeView />;
    case "project":
      return <ProjectView project={PROJECTS.find((p) => p.id === win.payload)} />;
    default:
      return <div style={{ padding: 16 }}>Unknown app.</div>;
  }
}
function statusFor(win) {
  switch (win.app) {
    case "files":
    case "projects":
      return ["⛁ C: drive", "Ready"];
    case "terminal":
      return ["● connected", "retro-sh 5.0"];
    case "readme":
      return ["📖 v1.0.95", "6 sections"];
    default:
      return null;
  }
}
...
function WelcomeWindow({ onLaunch }) {
  return (
    <div style={{ padding: 0, display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      >
        <div className="mono" style={{ fontSize: 14, opacity: 0.9, marginBottom: 2 }}>
          {">"} booting portfolio.exe …
        </div>
        <h1 className="chrome" style={{ margin: 0, fontSize: 30, textShadow: "2px 2px 0 rgba(0,0,0,0.3)" }}>
          {PERSONA.name}
        </h1>
      ...
      <div style={{ padding: "16px 20px", display: "grid", gap: 14, flex: 1 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          <RetroButton variant="primary" onClick={() => onLaunch("projects")}>
            🗂️ View Projects
          </RetroButton>
        ...
        >
          tip: double-click desktop icons · press <b>⌘K</b> for the command palette · open the <b>Design System</b> doc
          from Start
        </div>
      </div>
    </div>
  ...
  return (
    <div className="retro-scroll" style={{ overflow: "auto", height: "100%", background: "#4a3d6b", padding: 14 }}>
      ...
      >
        ...
        <div
          style={{ fontSize: 14, color: "#666", marginBottom: 14, borderBottom: "2px solid #1a1a1a", paddingBottom: 8 }}
        >
          {PERSONA.role} · {PERSONA.location} · {PERSONA.email}
        </div>
        <ResSec title="Summary">
          <p style={{ margin: 0, fontSize: 13, lineHeight: 1.5 }}>{PERSONA.blurb}</p>
        </ResSec>
        ...
        <ResSec title="Skills">
          <div style={{ fontSize: 12.5, lineHeight: 1.7 }}>
            {SKILLS.map((g) => (
              <div key={g.group}>
                <b>{g.group}:</b> {g.items.map((i) => i[0]).join(", ")}
              </div>
            ))}
          </div>
        </ResSec>
  ...
  return (
    <div
      ...
      style={{
        ...
        textShadow: "0 0 8px rgba(54,224,224,0.5)",
      }}
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 22 }}>
        ...
        <span
          className="chrome"
          style={{ fontSize: 26, color: "#ff5fd2", textShadow: "0 0 12px rgba(255,95,210,0.6)" }}
        >
          RetroOS
        </span>
      </div>
      {BOOT_LINES.slice(0, shown).map((l, i) => (
        <div key={i} style={{ color: l.accent ? "#ff5fd2" : "#36e0e0", minHeight: 26, whiteSpace: "pre-wrap" }}>
          {l.t}
        </div>
      ))}
      {shown < BOOT_LINES.length && <span style={{ animation: "boot-cursor 0.7s steps(1) infinite" }}>█</span>}
      <div className="mono" style={{ position: "absolute", bottom: 22, left: 48, fontSize: 14, color: "#6a5a92" }}>
...
function App() {
  ...
  const launch = (app, payload) => {
    wm.open(app, payload);
    setStartOpen(false);
  };
  const taskClick = (id) => {
    const w = wm.wins.find((x) => x.id === id);
    if (w.minimized || w.id !== wm.activeId) wm.focus(id);
    else wm.minimize(id);
  };
  ...
  return (
    <div
      ...
      onContextMenu={(e) => {
        if (e.target.closest("[data-desktop-root]") === e.currentTarget) {
        }
      }}
    >
      {/* desktop surface */}
      <div
        ...
        onContextMenu={(e) => {
          ...
          openMenu(e, [
            ...
            { sep: true },
            {
              label: t.theme === "dark" ? "Light theme" : "Dark CRT theme",
              icon: t.theme === "dark" ? "☀️" : "🌙",
              onClick: () => setTheme(t.theme === "dark" ? "light" : "dark"),
            },
            {
              label: t.crt === "off" ? "Enable CRT" : "Disable CRT",
              icon: "📺",
              onClick: () => setTweak("crt", t.crt === "off" ? "subtle" : "off"),
            },
            { sep: true },
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            textShadow: "0 0 14px rgba(255,95,210,0.6)",
            pointerEvents: "none",
          }}
        >
          <div style={{ fontSize: 28, letterSpacing: 1 }}>
            Retro<b style={{ color: "#36e0e0" }}>OS</b>
          </div>
          <div className="mono" style={{ fontSize: 13, opacity: 0.8 }}>
            portfolio kit · v1.0.95
          </div>
        </div>
      ...
      {/* shell chrome */}
      {startOpen && (
        <StartMenu onLaunch={launch} onClose={() => setStartOpen(false)} onPalette={() => setPaletteOpen(true)} />
      )}
      <Taskbar
        windows={wm.wins}
      ...
      <TweaksPanel>
        ...
        <TweakButton
          ...
          onClick={() => {
            ...
            wm.wins.forEach((w) => wm.close(w.id));
            setBooting(true);
          }}
        />
      </TweaksPanel>
