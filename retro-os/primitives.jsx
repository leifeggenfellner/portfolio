...
function RetroButton({ variant = "default", size = "md", active, disabled, className, children, ...rest }) {
  ...
  return (
    <button
      className={cx(
        "focus-ring no-select",
        variant !== "ghost" && "bevel-out",
        (pressed || active) && variant !== "ghost" && "bevel-pressed",
        className,
      )}
      style={base}
      disabled={disabled}
    ...
    >
      <span
        style={{
          transform: (pressed || active) && variant !== "ghost" ? "translate(1px,1px)" : "none",
          display: "inline-flex",
          alignItems: "center",
          gap: 6,
        }}
...
function RetroCard({ raised = true, className, style, children }) {
  return (
    <div
      className={cx(raised ? "bevel-out" : "bevel-in", className)}
...
function GroupBox({ label, children, style }) {
  return (
    <fieldset
      className="bevel-in-thin"
      style={{ position: "relative", margin: 0, padding: "16px 12px 12px", background: "transparent", ...style }}
    >
...
function WindowHeader({ icon, title, active = true, onMin, onMax, onClose, onPointerDown, maximized }) {
  return (
    ...
    >
      {icon && (
        <span style={{ fontSize: 15, lineHeight: 1, filter: active ? "none" : "grayscale(1) opacity(0.6)" }}>
          {icon}
        </span>
      )}
      <span
        style={{
          flex: 1,
          fontSize: 15,
          textShadow: active ? "1px 1px 0 rgba(0,0,0,0.35)" : "none",
          overflow: "hidden",
      ...
      <div style={{ display: "flex", gap: 3 }} onPointerDown={(e) => e.stopPropagation()}>
        ...
        {onMax && (
          <SysBtn label="Maximize" onClick={onMax}>
            {maximized ? "❐" : "▢"}
          </SysBtn>
        )}
        {onClose && (
...
function RetroWindow({
  ...
  const startResize = useCallback(
    (e) => {
      ...
      const sx = e.clientX,
        sy = e.clientY;
      const ow = winRef.current.offsetWidth,
        oh = winRef.current.offsetHeight;
      const move = (ev) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: "win-open 0.16s ease-out",
        padding: 3,
        ...style,
      }}
    >
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      <div
        role="tablist"
        onKeyDown={onKey}
        style={{ display: "flex", gap: 2, padding: "4px 4px 0", position: "relative", zIndex: 2 }}
      >
        {tabs.map((t) => {
      ...
      <div
        role="tabpanel"
        className="bevel-out retro-scroll"
        style={{
          flex: 1,
          minHeight: 0,
          overflow: "auto",
  ...
  return (
    <div
      ...
      style={{
        ...
        fontSize: 13,
        cursor: disabled ? "default" : "pointer",
        color: disabled ? "var(--text-disabled)" : h ? "var(--selection-text)" : "var(--text)",
  ...
  return (
    ...
    >
      ...
      >
        ...
        {actions && (
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, padding: "0 var(--sp-4) var(--sp-4)" }}>
            {actions}
          </div>
        )}
      </div>
