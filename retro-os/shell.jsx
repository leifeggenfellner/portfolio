...
function Clock() {
  ...
  return (
    <div
      className="bevel-in-thin no-select"
      style={{ padding: "3px 10px", fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}
    >
      <span title="Volume" style={{ cursor: "default" }}>
        🔊
      </span>
      <span title="Network" style={{ cursor: "default" }}>
        📶
      </span>
      ...
      <div style={{ flex: 1, display: "flex", gap: 4, overflow: "hidden" }}>
        {windows.map((win) => (
          <button
            ...
            className={cx(
              "bevel-out-thin no-select truncate",
              win.id === activeId && !win.minimized && "bevel-pressed",
            )}
            style={{
              minWidth: 130,
              maxWidth: 170,
  ...
  return (
    <div
      ...
      style={{
        ...
        left: 6,
        bottom: 44,
        zIndex: "var(--z-startmenu)",
        width: 280,
        background: "var(--surface)",
        padding: 3,
        display: "flex",
        boxShadow: "var(--win-shadow)",
        animation: "win-open 0.12s ease-out",
      }}
    >
  ...
  return (
    <div
      ...
      style={{
        ...
        color: h ? "var(--selection-text)" : "var(--text)",
      }}
    >
      ...
      {DOCK_ITEMS.map((d) => (
        <Tooltip key={d.id} label={d.label}>
          <button
            ...
            className="bevel-out focus-ring"
            style={{
              width: 46,
              height: 46,
              fontSize: 24,
...
function CommandPalette({ onLaunch, onClose, onTheme }) {
  ...
  const commands = [
    ...Object.keys(APP_REGISTRY)
      ...
      .map((k) => ({
        id: k,
        label: "Open " + APP_REGISTRY[k].title.split("—")[0].trim(),
        icon: APP_REGISTRY[k].icon,
        run: () => onLaunch(k),
      })),
    ...PROJECTS.map((p) => ({
      id: "prj-" + p.id,
      ...
      >
        ...
        <div style={{ padding: 6 }}>
          <input
            ...
            onKeyDown={onKey}
            placeholder="Open app, project, switch theme…"
            aria-label="command"
            className="bevel-in-thin"
            style={{ ...inputStyle, fontSize: 16, padding: "8px 10px" }}
          />
        </div>
        <div className="retro-scroll" style={{ overflow: "auto", padding: "0 6px 6px" }}>
          {filtered.length === 0 && (
            <div style={{ padding: 14, color: "var(--text-dim)", fontSize: 14 }}>No matches.</div>
          )}
          {filtered.map((c, i) => (
            <div
              key={c.id}
              ...
              style={{
                ...
                cursor: "pointer",
                fontSize: 14,
                background: i === sel ? "var(--selection-bg)" : "transparent",
                color: i === sel ? "var(--selection-text)" : "var(--text)",
              }}
            >
